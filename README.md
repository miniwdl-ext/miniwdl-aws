...
If the specified WDL source code is an existing local .wdl or .zip file, `miniwdl-aws-submit` automatically ships it in the workflow job definition as the WDL to execute. Given a .wdl file, it runs `miniwdl zip` to detect & include any imported WDL files; while it assumes .zip files were generated by `miniwdl zip`. If the source code is too large to fit in the AWS Batch request payload, then you'll need to pass it by reference to a URL or remote filesystem path.
