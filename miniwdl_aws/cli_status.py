...
def print_jobs(jobs):
    for job in jobs:
        start_time_str = datetime.utcfromtimestamp(job['createdAt']/1000).strftime('%Y-%m-%d %H:%M:%S') if job['createdAt'] else "----"
        stop_time_str = datetime.utcfromtimestamp(job['stoppedAt']/1000).strftime('%Y-%m-%d %H:%M:%S') if job['stoppedAt'] else "----"
