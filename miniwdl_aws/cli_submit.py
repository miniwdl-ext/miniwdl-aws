...
def zip_wdl(wdl_filename, verbose=False):
    ...
    code for the workflow job to execute. (Otherwise, it'll be passed through on the assumption
    it's some path/URI that'll be accessible to the workflow job itself.)
    ...
    # aggressively compress, to maximize the chance of fitting within the 32KB maximum Batch job
    # definition JSON
    import base64
    import lzma
    ...
    print(
        f"Encoded local {wdl_filename} to {len(zip_str)} characters to transmit with workflow job",
        file=sys.stderr,
    )
    return zip_str
